...
def analyze_opponent_xg_profile(team_name):
    """
    Analyze xG profile for an opponent across all their matches
    (excluding Göztepe matches).
    
    Returns:
        - season_profile: dict with aggregated xG metrics
        - match_analyses: list of per-match analysis dicts
    """
    ...
    for filename, df in match_dfs:
        result = _analyze_match_shots(df, team_name)
        if result:
            # Add match metadata
            teams = df['team_name'].unique().tolist()
            ...
            result['opponent'] = opponent[0] if opponent else 'Unknown'
    ...
    n_matches = len(match_analyses)

    # Aggregate season totals
    agg = {
        'total_shots': sum(m['total_shots'] for m in match_analyses),
        'total_on_target': sum(m['total_on_target'] for m in match_analyses),
        'total_goals': sum(m['total_goals'] for m in match_analyses),
        'total_xg': round(sum(m['total_xg'] for m in match_analyses), 3),
        'total_xgot': round(sum(m['total_xgot'] for m in match_analyses), 3),
        'big_chances': sum(m['big_chances'] for m in match_analyses),
        'header_shots': sum(m['header_shots'] for m in match_analyses),
    }

    # Aggregate origins
    agg_origins = {}
    agg_origin_xg = {}
    for key in ['open_play', 'from_cross', 'set_piece', 'fast_break', 'through_ball']:
        agg_origins[key] = sum(m['origins'][key] for m in match_analyses)
        agg_origin_xg[key] = round(sum(m['origin_xg'][key] for m in match_analyses), 3)
    ...
    agg_zone_sot = {}
    agg_zone_goals = {}
    for key in ['inside_box', 'outside_box']:
        agg_zones[key] = sum(m['zones'][key] for m in match_analyses)
        agg_zone_xg[key] = round(sum(m['zone_xg'][key] for m in match_analyses), 3)
        agg_zone_sot[key] = sum(m['zone_on_target'][key] for m in match_analyses)
        agg_zone_goals[key] = sum(m['zone_goals'][key] for m in match_analyses)
    ...
    season_profile = {
        ...
        **agg,
        ...
        # Per-game averages
        'shots_per_game': _safe_div(agg['total_shots'], n_matches),
        'xg_per_game': _safe_div(agg['total_xg'], n_matches),
        'xgot_per_game': _safe_div(agg['total_xgot'], n_matches),
        'goals_per_game': _safe_div(agg['total_goals'], n_matches),

        # Quality metrics
        'xg_per_shot': _safe_div(agg['total_xg'], ts, 3),
        'sot_pct': _safe_pct(agg['total_on_target'], ts),
        'conversion_pct': _safe_pct(agg['total_goals'], ts),
        'xgot_xg_ratio': _safe_div(agg['total_xgot'], agg['total_xg']) if agg['total_xg'] > 0 else 0,

        # Origin breakdown
        'origins': agg_origins,
        'origin_xg': agg_origin_xg,
        'origin_pcts': {k: _safe_pct(v, ts) for k, v in agg_origins.items()},
        'origin_xg_pcts': {k: _safe_pct(v, agg['total_xg']) for k, v in agg_origin_xg.items()},

        # Zone breakdown
        'zones': agg_zones,
        'zone_xg': agg_zone_xg,
        'zone_on_target': agg_zone_sot,
        'zone_goals': agg_zone_goals,
        'zone_pcts': {k: _safe_pct(v, ts) for k, v in agg_zones.items()},
        'zone_sot_pct': {
            'inside_box': _safe_pct(agg_zone_sot['inside_box'], agg_zones['inside_box']),
            'outside_box': _safe_pct(agg_zone_sot['outside_box'], agg_zones['outside_box']),
        },
        'zone_xg_per_shot': {
            'inside_box': _safe_div(agg_zone_xg['inside_box'], agg_zones['inside_box'], 3),
            'outside_box': _safe_div(agg_zone_xg['outside_box'], agg_zones['outside_box'], 3),
        },
        'zone_conversion': {
            'inside_box': _safe_pct(agg_zone_goals['inside_box'], agg_zones['inside_box']),
            'outside_box': _safe_pct(agg_zone_goals['outside_box'], agg_zones['outside_box']),
        },
    }

    return season_profile, match_analyses


def get_single_match_xg_analysis(filename, team_name):
    """
    Load a single match parquet file and return xG analysis.
    """
    data_dir = get_data_dir()
    ...
    try:
        ...
        print(f"Error loading {filename} for xG analysis: {e}")
        return None
